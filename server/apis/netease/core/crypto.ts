...
/** xeapi 签名密钥 */
const XEAPI_SIGN_KEY = "b1ced3e7b84e4c3f9c1ef8a7d6b2e4f1";
/** xeapi 静态密钥（密文外层 AES 加密） */
const XEAPI_STATIC_KEY = Buffer.from("0CoJUm6Qyw8W8jud");
/** X25519 公钥 SPKI DER 前缀（30 字节） */
...
/** AES-128-ECB 加密（Buffer → Buffer） */
const aesEcbEncrypt = (key: Buffer | string, plaintext: Buffer): Buffer => {
  const keyBuf = typeof key === "string" ? Buffer.from(key, "utf8") : key;
  const cipher = createCipheriv("aes-128-ecb", keyBuf, Buffer.alloc(0));
  return Buffer.concat([cipher.update(plaintext), cipher.final()]);
};

/** AES-128-ECB 解密（Buffer → Buffer） */
const aesEcbDecrypt = (key: Buffer | string, ciphertext: Buffer): Buffer => {
  const keyBuf = typeof key === "string" ? Buffer.from(key, "utf8") : key;
  const decipher = createDecipheriv("aes-128-ecb", keyBuf, Buffer.alloc(0));
