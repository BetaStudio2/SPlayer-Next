 ...
 */
export const getLrcByPlatformId = async (id: string): Promise<string | null> => {
  try {
    // 优先旧版 /api/song/lyric（_nmclfl:1 返回标准 LRC 文本）
    const { status, body } = await callNetease("lyric", { id });
    if (status === 200 && body.code === 200) {
      const lrc = body.lrc?.lyric?.trim();
      if (lrc && /^\[/.test(lrc)) return lrc;
    }
    // 回退新版 lyric_new
    const r2 = await callNetease("lyric_new", { id });
    if (r2.status === 200 && r2.body.code === 200) {
      const lrc2 = r2.body.lrc?.lyric?.trim();
      if (lrc2 && /^\[/.test(lrc2)) return lrc2;
    }
    return null;
  } catch (err) {
 ...
 * 复用 getByQuery 的搜索与匹配缓存，命中后单独取 LRC 字段。
 */
export const getLrcByQuery = async (track: Track): Promise<string | null> => {
  const fingerprint = buildFingerprint(track);
  let matchedId = getMatchedId(fingerprint, "netease")?.platformId;
  if (!matchedId) {
    // 触发搜索 + 缓存 id（结果中的 YRC 此处不用）
    await getByQuery(track);
    matchedId = getMatchedId(fingerprint, "netease")?.platformId;
  }
  if (!matchedId) return null;
  return getLrcByPlatformId(matchedId);
};
