/**
 * 第三方音源账号会话（cookies）存储
 *
 * 表：account_sessions(platform PK, cookies TEXT, updated_at)
 * - cookies 字段存 AES-256-GCM 密文（enc:v1: 前缀），解密后为 JSON 字符串
 * - 历史明文数据向后兼容：读取时非 enc:v1: 前缀原样解 JSON
 */

import { getDb } from "./index";
import { encryptString, decryptString } from "@main/utils/crypto";

/** 支持的音源平台标识 */
export type AccountPlatform = "netease";

interface SessionRow {
  platform: string;
  cookies: string;
  updated_at: number;
}

/** 读取某平台的 cookies；未登录返回空对象 */
export const getSessionCookies = (platform: AccountPlatform): Record<string, string> => {
  const row = getDb()
    .prepare("SELECT cookies FROM account_sessions WHERE platform = ?")
    .get(platform) as Pick<SessionRow, "cookies"> | undefined;
  if (!row) return {};
  try {
    // 解密（历史明文数据 decryptString 原样返回，兼容旧库）
    const json = decryptString(row.cookies);
    const parsed = JSON.parse(json) as Record<string, string>;
    return parsed ?? {};
  } catch {
    return {};
  }
};

/** 整体替换某平台的 cookies（登出传 {} 即可） */
export const saveSessionCookies = (
  platform: AccountPlatform,
  cookies: Record<string, string>,
): void => {
  const plaintext = JSON.stringify(cookies);
  const ciphertext = encryptString(plaintext);
  getDb()
    .prepare(
      `INSERT INTO account_sessions (platform, cookies, updated_at) VALUES (?, ?, ?)
       ON CONFLICT(platform) DO UPDATE SET
         cookies = excluded.cookies,
         updated_at = excluded.updated_at`,
    )
    .run(platform, ciphertext, Date.now());
};

/** 清除某平台的 cookies（登出） */
export const clearSessionCookies = (platform: AccountPlatform): void => {
  saveSessionCookies(platform, {});
};

